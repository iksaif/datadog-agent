...
func (l *DockerListener) createContainerService(container *workloadmeta.Container) {
	// TODO(juliogreff): test that image exclusion still works
	// TODO(juliogreff): this uses rawName, but logs name
	...
	if l.filters.IsExcluded(
		...
		log.Debugf("container %s filtered out: name %q image %q", container.ID, container.Name, containerImg.Name)
		return
	}
