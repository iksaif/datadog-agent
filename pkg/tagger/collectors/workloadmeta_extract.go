...
func (c *WorkloadMetaCollector) handleContainer(ev workloadmeta.Event) []*TagInfo {
	...
	c.extractFromMapWithFn(container.Labels, c.containerLabelsAsTags, tags.AddAuto)
	...
	// extract env as tags
	c.extractFromMapWithFn(container.EnvVars, c.containerEnvAsTags, tags.AddAuto)

	low, orch, high, standard := tags.Compute()
	tagInfos = append(tagInfos, &TagInfo{
		Source:               fmt.Sprintf("%s-%s", workloadmetaCollectorName, string(workloadmeta.KindContainer)),
...
func (c *WorkloadMetaCollector) handleKubePod(ev workloadmeta.Event) []*TagInfo {
	...
	low, orch, high, standard := tags.Compute()
	tagInfos = append(tagInfos, &TagInfo{
		Source:               fmt.Sprintf("%s-%s", workloadmetaCollectorName, string(workloadmeta.KindKubernetesPod)),
	...
	for _, containerID := range pod.Containers {
		...
		tagInfos = append(tagInfos, &TagInfo{
			// KindKubernetesPod here is not a mistake. the source
			// is always from the parent resource.
			Source:               fmt.Sprintf("%s-%s", workloadmetaCollectorName, string(workloadmeta.KindKubernetesPod)),
			Entity:               buildTaggerEntityID(container.EntityID),
			HighCardTags:         high,
			OrchestratorCardTags: orch,
...
func (c *WorkloadMetaCollector) extractFromMapWithFn(input map[string]string, mapping map[string]string, fn func(string, string)) {
	for key, tag := range mapping {
		if value, ok := input[key]; ok && value != "" {
			fn(tag, value)
		}
	}
}
