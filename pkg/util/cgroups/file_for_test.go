...
func (cfs *cgroupMemoryFS) setFileContent(path, content string) {
	cfs.files[filepath.Join(cfs.rootPath, path)] = content
}

func (cfs *cgroupMemoryFS) deleteFile(path string) {
	delete(cfs.files, filepath.Join(cfs.rootPath, path))
}
